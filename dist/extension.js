...
function activate(context) {
    let disposable = vscode.commands.registerCommand('extension.fetchTestCase', async () => {
        ...
        (0, child_process_1.exec)(`python3 ${scriptPath} --titleSlug ${titleSlug}`, (error, stdout, stderr) => {
            ...
            const panel = vscode.window.createWebviewPanel('testCaseOutput', 'Test Case Output', vscode.ViewColumn.One, {
...
function run_cpp_file(context, panel) {
    ...
    (0, child_process_1.exec)(`g++ -o "${executablePath}" "${cppFilePath}"`, (compileError, stdout, stderr) => {
        ...
        const pythonProcess = (0, child_process_1.spawn)('python3', [pythonScriptPath, executablePath, testCaseDir], { shell: true });
        let output = '';
        let errorOutput = '';
...
function run_python_file(context, panel) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showErrorMessage('No workspace folder is open. Cannot run test cases.');
        return;
    ...
    const pythonProcess = (0, child_process_1.spawn)('python3', [pythonScriptPath, pythonFilePath, testCaseDir], { shell: true });
    let output = '';
    let errorOutput = '';
    pythonProcess.stdout.on('data', (data) => {
        output += data.toString();
...
function openPythonTab(context) {
